...
class Query(object):
    ...
    def where(self, *q_or_node):
        ...
        if self._where is None:
            ...
            print(' self._where1:  ', self._where)
        else:
...
class ModelOptions(object):
    ...
    def get_sorted_fields(self):
        ...
        return sorted(self.fields.items(), key=lambda x: (x[0] is self.primary_key and 1 or 2, x[1]._order))
...
class Model(metaclass=BaseModel):
    ...
    def update(cls, **update):
        ...
        # fdict= dict([(cls._meta.fields[f], update[f]) for f in update])
        # fdict = {}
    ...
    def insert(cls, **insert):
        ...
        # fdict = dict([(cls._meta.fields[f], insert[f]) for f in insert])
        # fdict = {}
...
def sort_models_topologically(models):
    ...
    # order model by name and table initially to guarantee a total ordering
...
# new_user = User(name='LiMing98', age="8")
if __name__ == "__main__":
    ...
    # for row in r:
    #     print('row：', row.age)
    # q = User.update(age=21).where(User.name == 'LiMingt').execute()
    # print(q)
    # 基本上，你的句子的第二部分是不正确的。 name是自定义类型。 它的__eq__方法不仅返回一个布尔值，而是一个包含实际完成比较的信息的对象。 （这个类甚至可能来自bool所以它在其他上下文中就像一个布尔。）它的其他丰富的比较方法是相似的。
    # 但是Person.name如何知道它的名字是name ？ 答案是， Person可能实际上没有name属性。 相反，它有一个__getattr__()
    # 方法返回name对象，它是具有我刚刚描述的__eq__方法的自定义类。
    # 由于Person.__getattr__()
    # 接收属性的名称，因此它可以将该名称烘焙到它为name返回的值。 然后name上的自定义__eq__方法返回一个类似布尔的对象，其中包含name == 'Grandma L.'
    # 某些表示形式name == 'Grandma L.'
    # 在里面。 Person.get()
    # 使用此表示将查询放在一起。
    # r = new_user.select().get().name
    # print(r)
    # User.delete().where(User.name == 'LiMing').execute()
    r = Manager.select().where(Manager.login_name == 'admin')#.execute()

    for row in r:
        print('ro', row.name)
    print('rw ', r.count())
    # Manager.login_count=21
    # fields = {
    #    'last_login_time':'now()',

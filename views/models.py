...
def insert_datas():
    ...
    for g in Group.select().where(Group.name.startswith('销售')):
           g.roles.add(Role.get(name='职员'))
    # Role.get(name="总监").menus.add(Menu.select())
    Role.get(Role.name == '总监').menus.add(Menu.select().where(Menu.name.startswith('销售')))

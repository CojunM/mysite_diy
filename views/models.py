...
class User(BaseModel):
    login_name = CharField(max_length=32, help_text='登陆账号')
...
class Menu(BaseModel):
    ...
    菜单
    """
    name = CharField(max_length=30, unique=True, verbose_name="菜单名",
                     help_text='菜单名称或各个页面功能名称')  # unique=True, 这个字段在表中必须有唯一值.
    parent_id = ForeignKeyField('self', null=True, on_delete="SET NULL", verbose_name="父菜ID")
    ...
    page_url = CharField(max_length=128, unique=True, null=True, help_text='各页面URL（主菜单与分类菜单没有URL）',
                         verbose_name="页面URL")
    interface_url = CharField(max_length=128, unique=True, null=True, help_text='各接口url', verbose_name="页面接口URL")
    sort = IntegerField(default=0, help_text='排序')
    level = IntegerField(default=0, help_text='树列表深度级别，即当前数据在哪一级')
    is_leaf = BooleanField(default=False, help_text='是否最终节点')
    expanded = BooleanField(default=False, help_text='此节点是否展开，后台菜单列表js要用到，不用进行编辑')
...
class Permission(BaseModel):
    caption = CharField(max_length=32)
    url = CharField(max_length=32)
    menu = ForeignKeyField(Menu, null=True, on_delete='CASCADE')

...
def callback():
    ...
    if sidx:
        order_by = getattr(getattr(Manager, sidx), sord)()

    _manager_logic =db_logic(Manager)
    # 读取记录
    result = _manager_logic.get_list('', '', page_number, page_size, order_by)
    lst = [{'id': r.id, 'login_name': r.login_name, 'last_login_time': r.last_login_time, 'last_login_ip': r. last_login_ip , 'login_count': r.login_count,
            'create_time': r.create_time, 'department_id': r.department_id, 'department_name': r.department_name, 'positions_name': r.positions_name, 'is_work': r.is_work,
            'is_enabled': r.is_enabled, 'sex': r.sex , 'name': r.name} for r in [m for m in result.get('rows')]]
    result['rows'] = lst
    if result:
